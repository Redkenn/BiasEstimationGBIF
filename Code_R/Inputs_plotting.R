...
names(inputs)[4] <- "MR"
names(inputs)[5] <- "TotalSE"
names(inputs)[6] <- "WikiW"
names(inputs)[7] <- "PlantW"
names(inputs)[8] <- "OthersW"
names(inputs)[9] <- "iNaturW"

# define function to scale values between 0 and 1
scale_values <- function(x){(x-min(x))/(max(x)-min(x))}

# scale Total_SE variable
inputs$TotalSE <- scale_values(inputs$TotalSE)
...
p4  <- 
  ggplot(inputs)+
  geom_sf(aes(fill = TotalSE))+
  scale_fill_viridis('TotalSE',option='viridis',direction = -1)+
  labs(title = "TotalSE", x="Longitude", y="Latitude", fill = "TotalSE") +theme_light()+
  theme(legend.background=element_blank(),
        panel.grid = element_blank(),
        legend.position = 'bottom',
        plot.title = element_text(size=17,face = 'bold',hjust = 0.5),
...
ptot2 <- p5 + p6 + p7 + p8 + plot_annotation(tag_levels = list(c('E','F','G','H'))) + plot_layout(nrow = 1, ncol = 4)
