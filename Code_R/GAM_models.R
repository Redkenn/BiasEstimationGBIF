...
inputs <- st_read("Article_BIAS/Bias_data/SHP/Inputs/g_inputs.shp") %>% st_transform(32632)
...
df <- data.frame(inputs$id, inputs$rd_dnst, inputs$sdNDVI,inputs$mn_rghn, inputs$Totl_SE, inputs$W_Wk_SE,
...
gam_Comp2 <- gam(Comp ~ s(RD) + s(sdNDVI)  + s(MR) + s(TotalSE) + s(Lon, Lat), data = df, method = 'REML', select = TRUE)
gam_Comp3 <- gam(Comp ~ s(RD) + s(sdNDVI)  + s(MR) + s(WikiW) + s(Lon, Lat), data = df, method = 'REML', select = TRUE)
gam_Comp4 <- gam(Comp ~ s(RD) + s(sdNDVI)  + s(MR) + s(PlantW) + s(Lon, Lat), data = df, method = 'REML', select = TRUE)
gam_Comp5 <- gam(Comp ~ s(RD) + s(sdNDVI)  + s(MR) + s(OthersW) + s(Lon, Lat), data = df, method = 'REML', select = TRUE)
gam_Comp6 <- gam(Comp ~ s(RD) + s(sdNDVI)  + s(MR) + s(iNaturW) + s(Lon, Lat), data = df, method = 'REML', select = TRUE)



# model comparison
AIC(gam_Comp1,gam_Comp2,gam_Comp3,gam_Comp4,gam_Comp5,gam_Comp6)
...
gam_Jtemp6 <-  gam(Jtemp ~ s(RD) + s(sdNDVI) + s(MR) + s(iNaturW) + s(Lon, Lat), data = df, method = 'REML', select = TRUE)


AIC(gam_Jtemp1,gam_Jtemp2,gam_Jtemp3,gam_Jtemp4,gam_Jtemp5,gam_Jtemp6)
...
gam_NNI6 <- gam(NNI~ s(RD) + s(sdNDVI)  + s(MR) + s(iNaturW) + s(Lon,Lat), data = df, method = 'REML', select = TRUE)


# Model comparison
AIC(gam_NNI1,gam_NNI2,gam_NNI3,gam_NNI4,gam_NNI5,gam_NNI6)
